...
int contador = 0;
unsigned long n=0, senha = 0, senha2 = 0, tempo=0;
...
byte rowPins[ROWS] = {4, 5, 6, 7};  			// Define os pinos do arduino conectados as linhas
byte colPins[COLS] = {8, 9, 10};     			// Define os pinos do arduino conectados as colunas
...
void loop()
{
	...
	if ( key )								// Entra no if quando uma tecla é pressionada
	{
		Serial.println(key);
		tone(som, 400, 200);
		delay(50);							// Fim da inserção da senha ( hora de checar o valor)
		//***************************************************************************************
		if ( key >=48 && key <= 58 )		// Checa se o valor apertado é um número
		{
			senha = 10*senha + ( key - 48 );// incrementa n
			tempo = millis();				// zera o valor do tempo	
			contador = contador + 1;
		}
